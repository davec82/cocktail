defmodule Cocktail.Validation.MonthOfYear do
  ...
  def next_time(%__MODULE__{months: months}, time, start_time) do
    current_time = maybe_update_current_time(time, start_time)
    ...
    days_diff =
      case time.day == start_time.day and time.month in normalized_months do
        true ->
          0

        _ ->
          new_time = find_next_month_day(current_time, start_time, normalized_months)
  ...
  defp maybe_update_current_time(time, start_time) do
    case time.day > start_time.day do
      true ->
        shift_to_first_month_day(time)
  ...
  defp maybe_shift_month(time, start_time, normalized_months) do
    ...
    case time.month in normalized_months do
      true -> Timex.set(time, day: start_day)
